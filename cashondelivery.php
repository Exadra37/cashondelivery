...
if(!parent::install() OR !$this->registerHook('payment') OR !$this->registerHook('paymentReturn'))return false;
return true;}
...
$smarty->assign(array('dobirecne' => $this->l('Free!')));}
else{
$smarty->assign(array('dobirecne' =>$this->l('The fee is '). Tools::displayPrice (Tools::convertPrice($this->fee))));}
...
if($fee>$cart_total_paid){$fee=Tools::convertPrice($this->fee,$order->id_currency)/100;}
if($fee<$RecargoMinimo){$fee=$RecargoMinimo;}
/*//////////////////////////////////////////////////////////
...
if (($order_status->logable && number_format($cart_total_paid+$fee, 2) != number_format($amount_paid+$fee, 2))and($order_status->logable && number_format($cart_total_paid+$fee, 2) != number_format($amount_paid, 2))){$id_order_state = Configuration::get('PS_OS_ERROR');}
...
$virtual_product &= false;} // end foreach ($products)
